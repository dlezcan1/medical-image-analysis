...
train_gt_ds = pixelLabelDatastore(fullfile(train_dir, 'label'), ...
        {'background', 'heart'},...
        [0, 1], 'IncludeSubfolders', true,...
        'FileExtensions', '.gz');
train_gt_ds.ReadFcn = @(f) nifti_gtreader(f, img_size(1:3), 'heart');
...
val_gt_ds = pixelLabelDatastore(fullfile(val_dir, 'label'), ...
        ...
        [0, 1], 'IncludeSubfolders', true,...
        'FileExtensions', '.gz');
val_gt_ds.ReadFcn = @(f) nifti_gtreader(f, img_size(1:3), 'heart');
